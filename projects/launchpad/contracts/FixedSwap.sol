...
contract FixedSwap is Base {
    ...
    function _swap(uint256 index, uint256 amount1) private {
        ...
        uint256 _amount1 = pool.amountTotal1 - amountSwap1P[index];
        if (_amount1 < amount1) {
            excessAmount1 = amount1 - _amount1;
        } else {
            _amount1 = amount1;
        }

        // check if amount0 is exceeded
        uint256 amount0 = (_amount1 * (pool.amountTotal0)) / (pool.amountTotal1);
        uint256 _amount0 = pool.amountTotal0 - amountSwap0P[index];
        if (_amount0 > amount0) {
            _amount0 = amount0;
        }

        amountSwap0P[index] = amountSwap0P[index] + (_amount0);
        amountSwap1P[index] = amountSwap1P[index] + (_amount1);
        myAmountSwapped0[msg.sender][index] = myAmountSwapped0[msg.sender][index] + (_amount0);
        myAmountSwapped1[msg.sender][index] = myAmountSwapped1[msg.sender][index] + (_amount1);
        ...
        if (pool.claimAt == 0) {
            if (_amount0 > 0) {
                ...
                IERC20(pool.token0).transfer(msg.sender, _amount0);
            }
        }
        if (excessAmount1 > 0) {
            ...
            if (pool.token1 == address(0)) {
                ...
                IERC20(pool.token1).transfer(msg.sender, excessAmount1);
            }
        }
    ...
    function creatorClaim(uint256 index) external nonReentrant isPoolExist(index) {
        ...
        uint256 txFee = (amountSwap1P[index] * txFeeRatio) / TX_FEE_DENOMINATOR;
        uint256 _amount1 = amountSwap1P[index] - txFee;
        if (_amount1 > 0) {
            if (pool.token1 == address(0)) {
                payable(pool.creator).transfer(_amount1);
            } else {
                IERC20(pool.token1).transfer(pool.creator, _amount1);
            }
        }
        ...
        if (unSwapAmount0 > 0) {
            IERC20(pool.token0).transfer(pool.creator, unSwapAmount0);
        }
    ...
    function userClaim(uint256 index) external nonReentrant isPoolExist(index) isClaimReady(index) {
        ...
        if (releaseDataList[index].length > 0) {
            ...
            if (releaseAmount > 0) {
                uint256 actualReleaseAmount = releaseAmount - myReleased[msg.sender][index];
                if (actualReleaseAmount > 0) {
                    IERC20(pool.token0).transfer(msg.sender, actualReleaseAmount);
            ...
            if (myAmountSwapped0[msg.sender][index] > 0) {
                // send token0 to msg.sender
                IERC20(pool.token0).transfer(msg.sender, myAmountSwapped0[msg.sender][index]);
            }
            myClaimed[msg.sender][index] = true;
    ...
    function reverse(uint256 index, uint256 amount0) external nonReentrant isPoolExist(index) isPoolNotClosed(index) {
        ...
        uint256 amount1 = (amount0 * myAmountSwapped1[msg.sender][index]) / myAmountSwapped0[msg.sender][index];
        myAmountSwapped0[msg.sender][index] = myAmountSwapped0[msg.sender][index] - amount0;
        myAmountSwapped1[msg.sender][index] = myAmountSwapped1[msg.sender][index] - amount1;
        amountSwap0P[index] = amountSwap0P[index] - amount0;
        amountSwap1P[index] = amountSwap1P[index] - amount1;
